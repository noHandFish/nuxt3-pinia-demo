<template>
  <div v-if="!userData.isAdmin">
    ...
    <div class="divide-y m-4 outline outline-1 outline-gray-200 rounded-sm">
      <div v-for="(item, index) in toDoListStore.toDoList" :key="index" class="flex items-center justify-between p-4">
        <div>{{ item.content }}</div>
        <DeleteIcon @click="del(index)" class="cursor-pointer"/>
      </div>
    </div>
  ...
  <div v-else>
    ...
    <div class="divide-y m-4 outline outline-1 outline-gray-200 rounded-sm">
      <div v-for="(item, index) in toDoList" :key="index" class="p-4">
        <div>{{ item.content }}</div>
      </div>
    </div>
    <t-dialog v-model:visible="visible" @cancel="cancelAdd" @confirm="add">
...
let toDoList = reactive([] as toDoListItem[])
